...
def test_publish_concurrent_replaced_header_during_repair(monkeypatch):
    r"""Test header replaced by another process while repairing files.

    If another process replaces our header
    after we started to upload replaced files again,
    its header registers the model.
    We have to stop uploading files,
    and raise an error.
    ...
    """
    name = pytest.NAME
    params = {"replaced": "header-during-repair"}
    version = "1.0.0"
    subgroup = f"{SUBGROUP}.replaced"
    uid = audmodel.uid(name, params, version, subgroup=subgroup)
    short_id = uid.split("-")[0]
    ...
    def exists_and_replace_meta(uid):
        r"""Replace meta after the check performed before publishing header."""
        result = original_exists(uid)
        exists_calls.append(uid)
        if len(exists_calls) == 2:
            original_put_meta(short_id, version, other_meta, backend_interface, False)
        return result

    def put_meta_and_replace_header(*args):
        r"""Replace header when meta is uploaded again."""
        path = original_put_meta(*args)
        put_meta_calls.append(path)
        if len(put_meta_calls) == 2:
            api.put_header(short_id, version, other_header, backend_interface, False)
            original_put_meta(short_id, version, other_meta, backend_interface, False)
        return path

    monkeypatch.setattr(api, "exists", exists_and_replace_meta)
    monkeypatch.setattr(api, "put_meta", put_meta_and_replace_header)
...
def test_publish_concurrent_replaced_repeatedly(monkeypatch):
    r"""Test file replaced by another process after every repair.

    If a file is replaced again
    after every upload,
    we stop after a few rounds,
    and warn that the published model
    might not contain the correct files.
    ...
    repository = pytest.REPOSITORIES[0]
    backend_interface = repository.create_backend_interface()
    other_meta = {"author": "other-process"}
    ...
    def put_meta_and_replace_meta(*args):
        r"""Replace meta again after every repair."""
        path = original_put_meta(*args)
        put_meta_calls.append(path)
        if len(put_meta_calls) > 1:
            original_put_meta(short_id, version, other_meta, backend_interface, False)
        return path

    monkeypatch.setattr(api, "exists", exists_and_replace_meta)
    monkeypatch.setattr(api, "put_meta", put_meta_and_replace_meta)

    warning_msg = (
        "The files were uploaded again, but were replaced again afterwards. "
        "Check that the published model contains the correct files."
    )
    with pytest.warns(UserWarning, match=warning_msg):
        assert (
            audmodel.publish(
    ...
    # Initial upload plus one repair per verification round
    rounds = audmodel.core.define.PUBLISH_VERIFICATION_ROUNDS
    assert len(put_meta_calls) == 1 + rounds
    assert audmodel.exists(uid)
    assert audmodel.meta(uid) == other_meta
